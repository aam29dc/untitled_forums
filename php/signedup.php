...
if($_SERVER["REQUEST_METHOD"] === "POST"){      
    ...
    if(tableExists($pdo,'users')) {
        ...
        if(!$error){
            ...
            if($stmt->execute()){
                $report .= "<p>Successfully added user: ".$_POST['s_username']."</p><span>Welcome to the forums, feel free to submit an article.</span>";
...
if($error) echo '<script src="'.abs_php_include($x).'js/waitdirect.js"></script><script>waitdirect(2000, "'.abs_php_include($x).'index.php?page=signup");</script>';
