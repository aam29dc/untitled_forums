    ...
    if($stmt->execute()){
        ...
        while($row = $stmt->fetch(PDO::FETCH_ASSOC)){
            ...
            if($stmt2->rowCount() > 0){
                ...
                if((time() < strtotime($ban) + 14400) && isset($ban)) echo $ban;
                else echo "unbanned";
            }
            echo "</td></tr>";
        }
        ...
        // BUTTON: PREV
        if($q['pages'] > 1){
            echo '<a class="nsyn" href="?page=cp_users&pages='.($q['pages']-1).'"><button>&laquo; Prev</button></a>';
        }

        // BUTTON: NEXT
        if($count > UMAX && $stmt->rowCount() === UMAX){
            echo '<a class="nsyn" href="?page=cp_users&pages='.($q['pages']+1).'"><button>Next &raquo;</button></a>';
        }
