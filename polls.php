...
if(!is_numeric($pages) || empty($pages) || $pages < 1) $pages = 1;   // if query string is not numeric, then page=1;

//save page for when user hits, back, inside a page
//$_SESSION['pollpage'] = $pages;

require_once('php/lib.php');
require_once('php/conn.php');

if(tableExists($pdo,'polls')){
    $stmt = $pdo->prepare("SELECT pollid, question, authorid, date FROM polls ORDER BY pollid DESC LIMIT ?, ".TMAX.";");  // ? = ($pages-1)*10
    $stmt->bindValue(1, (int)(($pages-1)*TMAX), PDO::PARAM_INT);
    $stmt->execute();
    ...
    if($stmt->rowCount() > 0){
        while($row = $stmt->fetch(PDO::FETCH_ASSOC)){
            ...
            if($blocked) echo "Blocked question.";
